 ...
 */
@WebServlet("/ReturnRequestController")
public class ReturnRequestController extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		boolean check = false;
		try {
			check = bookLendingApplicationDAO.returnedRequest(requestId,bookId);
		...
		if(check == true) {
			...
			response.getWriter().write("Book Returned");
		}
		else if(check == false){
			response.setContentType("text/plain");
			response.getWriter().write("Error in Returning Book Request");
		}
	}
